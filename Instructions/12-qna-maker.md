...
ナレッジ ベースを作成してホストするには、Azure サブスクリプションに QnA Maker リソースが必要です。

1. `https;//portal.azure.com` で Azure portal を開き、Azure サブスクリプションに関連付けられている Microsoft アカウントを使用してサインインします。
2. **「リソースの作成」** ボタンを選択し、*QnA* を検索して、次の設定で **QnA Maker** リソースを作成します。
    - **マネージド**: オン
    ...
    - **場所**: *使用可能な場所を選択します*
    - **価格レベル**: Standard S0
    - **Azure 検索の場所**\*：*QnA Maker リソースと同じグローバル リージョン内の場所を選択します*。
    - **Azure Search の価格レベル**：無料 (F) (*このレベルが利用できない場合は、「基本 (B)」 を選択します*)

    \*QnA Maker は、Azure Search を使用して、質問と回答のナレッジ ベースにインデックスを付けてクエリを実行します。

3. 法的条件のチェックボックスを選択し、リソースを作成します。
4. デプロイが完了するのを待ってから、デプロイの詳細を表示します。

## ナレッジ ベースの作成

QnA Maker リソースにナレッジ ベースを作成するには、QnA Maker ポータルを使用できます。この場合、[Microsoft Learn](https://docs.microsoft.com/learn) に関する質問と回答を含むナレッジ ベースを作成します。
...
3. QnA Maker リソースは既に作成されているため、手順 1 をスキップできます。**手順 2** のセクションで、次の設定を選択します。
    - **Microsoft Azure ディレクトリ ID**: サブスクリプションを含む Azure ディレクトリ。
    - **Azure サブスクリプション名**: Azure サブスクリプション。
    - **Azure QnA サービス**: 以前に作成した QnA Maker リソース。
...
5. コピーしたコードをターミナルに貼り付け、編集して **lt;your question&gt;**を「**What is a learning path?**」に置き換えます。 
